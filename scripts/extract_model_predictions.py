...
tf.app.flags.DEFINE_string('dataset_name', 'v0', 'semantic segmentation dataset name')
tf.app.flags.DEFINE_string('job_name', 'v0', 'name for the experiment')
tf.app.flags.DEFINE_integer('region_num_per_image', 1024, 'Number of regions per image.')
tf.app.flags.DEFINE_integer('num_superpixels', 1024, 'Number of superpixels per image.')
tf.app.flags.DEFINE_integer('region_size', 32, 'region size')
tf.app.flags.DEFINE_integer('batch_id', 0, 'batch id')
tf.app.flags.DEFINE_string('region_type', 'v0', 'rec or sp')
...
def compute_sp_pseudo_label_uncertainty(image_list, image_name_path, num_class):
    ...
    superpixel_label_dir = './superpixels/%s/%s_%d/%s/label'%(FLAGS.dataset_name, FLAGS.sp_method, num_superpixels, FLAGS.split)
            ...
            for j in sp_dic['valid_idxes']:
                ...
                if y.size > 0:
                    if FLAGS.is_bal == 'True':
                        ...
                        class_to_region_idx[class_id].append(j+ n * region_num_per_image)                               
