...
def randomly_scale_image_and_label_sp(image, label=None, sp_label = None, scale=1.0):
  ...
  if sp_label is not None:
    sp_label = tf.squeeze(tf.image.resize_nearest_neighbor(
        tf.expand_dims(sp_label, 0),
        new_dim,
        align_corners=True), [0])

  return image, label, sp_label
...
def resize_to_range_sp(image,
  ...
  with tf.name_scope(scope, 'resize_to_range', [image]):
    ...
    if sp_label is not None:
      if label_layout_is_chw:
        # Input label has shape [channel, height, width].
        resized_label_sp = tf.expand_dims(sp_label, 3)
        resized_label_sp = tf.image.resize_nearest_neighbor(
            resized_label_sp, new_size, align_corners=align_corners)
        resized_label_sp = tf.squeeze(resized_label_sp, 3)
      else:
        # Input label has shape [height, width, channel].
        resized_label_sp = tf.image.resize_images(
            sp_label, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR,
            align_corners=align_corners)
      new_tensor_list.append(resized_label_sp)
    else:
      new_tensor_list.append(None)
    return new_tensor_list
