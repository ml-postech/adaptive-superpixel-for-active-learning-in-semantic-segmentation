...
flags.DEFINE_string('accuracy_log', None, 'Directory of model checkpoints.')
flags.DEFINE_string('batch_log', None, 'Directory of model checkpoints.')
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.batch_id == 0:
       f = open(FLAGS.accuracy_log,"w")
    else:
       f = open(FLAGS.accuracy_log,"a")

    f.write("batch %d mIOU is:%f\n" % (FLAGS.batch_id, a['miou_1.0']))

    for index in range(0,dataset.num_classes):
        class_iou = 'class_'+str(index)+'_iou'
        f.write("batch %d %s is:%f\n" % (FLAGS.batch_id, class_iou, a[class_iou]))
    f.close()
    
    f = open(FLAGS.batch_log,"w")
    f.write("%d" % FLAGS.batch_id)
