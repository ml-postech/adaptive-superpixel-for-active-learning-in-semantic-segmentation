...
#from crf_rnn_loss_layer import crf_rnn_loss_layer
...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    scaled_labels_flatten = tf.reshape(scaled_labels, shape=[-1])  
    not_ignore_mask = tf.to_float(tf.not_equal(scaled_labels_flatten,
                                               ignore_label)) * loss_weight
    one_hot_labels = slim.one_hot_encoding(
        scaled_labels_flatten, num_classes, on_value=1.0, off_value=0.0)
